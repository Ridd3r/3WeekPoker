...
int player3Balance = 0;
int player4Balance = 0;
int player5Balance = 0;
int player6Balance = 0;
int player7Balance = 0;
int player8Balance = 0;
...
int foldArray[8] = {0, 0, 0, 0, 0, 0, 0, 0};
int nonplayStatus[8] = {0, 0, 0, 0, 0, 0, 0, 0};
	...
	for(int i = 0; i < GRANUALITYHEIGHT; i++){
		for(int j = 0; j < GRANUALITYWIDTH; j++){
			...
			if(locationArray[i][j] >= 53 && locationArray[i][j] <= 60 ){ //show icons
				...
				showChip(*(betValArray[(locationArray[i][j]-61)]), j, i, 0xFF0000);
				...
				showCard(53, j,  i);
			}else if(locationArray[i][j] == 78){
				showChip(poolval, j, i, 0xFFFF00);
			}
		}
	}
...
void showICON(int player,int x, int y, int highlight){
	...
	if(foldArray[player] == 1){
		backgroundcolor = 0xFF0000;
	}
	if(nonplayStatus[player] == 1){
...
void showChip(int value, int x, int y, int color){
	int blockwidth = dispCtrl.vMode.width/GRANUALITYWIDTH;
	int blockheight = dispCtrl.vMode.height/GRANUALITYHEIGHT;
	...
	for(int i = 0; i < 60; i++){
		for(int j = 0; j < 60; j++){
			if(chip[i][j] == 0){
				...
				drawSquare2(blockwidth,blockheight, (x + j)*blockwidth, (y + i)*blockheight, color, dispCtrl.framePtr[nextFrame()]);
			}
			else if(chip[i][j] == 4){
...
void currentPlayerICON(int player){
...
void initChips(int player){
	placeObject(PlayerChipCoords[player][0], PlayerChipCoords[player][1], (61 + player));

}
void poolChip(){
	placeObject(285, 75 + 60+30, 78);
}
void removeChips(){
	for (int i = 0; i < 8; i++) {
		placeObject(PlayerChipCoords[i][0], PlayerChipCoords[i][1], 0);
	}
	//placeObject(285, 75 + 60+30, 0);//poolchip
}
void removePoolChip(){
	placeObject(285, 75 + 60+30, 0);//poolchip
}
...
void initBalanceSheets(){
	...
	for(int i = 0; i < AmountOfPlayers; i++ ){
		if(nonplayStatus[i] == 0){
			placeObject(balanceCoords[i][0], balanceCoords[i][1], sheetVal);
		}else{
			placeObject(balanceCoords[i][0], balanceCoords[i][1], 0);
		}
		sheetVal++;
	}
}
...
void clearDealerCards(){
		...
		for (int i = 0; i < 8; i++) {
		placeObject(PlayerCardCoords[i][0],PlayerCardCoords[i][1], 0);
		placeObject(PlayerCardCoords[i][2],PlayerCardCoords[i][3], 0);
		}
}
...
void fillInCardArray(){
	...
	memcpy(cardGraphicArray[rankSuitToIndex(13, 1)], kingOfHearts ,cardsSize);
...
void updateFoldstatus(int player, int i ){
	foldArray[player] = i;
}
...
void removePlayerFromGame(int player){
	placeObject(PlayerChipCoords[player][0],PlayerChipCoords[player][1], 0);
	placeObject(balanceCoords[player][0],balanceCoords[player][1], 0);
	clearPlayerCards(player);
	nonplayStatus[player] = 1;
}
//empties both frames
void resetBothframes(){
	memcpy(dispCtrl.framePtr[nextFrame()], background, DEMO_MAX_FRAME);
	memcpy(dispCtrl.framePtr[dispCtrl.curFrame], background, DEMO_MAX_FRAME);
}
void printText(){}
